...
function sendBpl (account, i, done) {
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomBpl / node.normalizer;
		}
		done();
	});
}

function sendBplFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	sendBpl(multisigAccount, null, done);
});
...
describe('PUT /api/api/transactions/', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendBplFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
